...
CLASS zcl_load_global_sales IMPLEMENTATION.
  ...
  METHOD load_master.
    DATA lt_curr TYPE STANDARD TABLE OF z_currency.
    lt_curr = VALUE #(
    ...
    DELETE FROM z_currency WHERE client = @sy-mandt.
    INSERT z_currency FROM TABLE @lt_curr.

    DATA lt_country TYPE STANDARD TABLE OF z_country.
    lt_country = VALUE #(
    ...
    DELETE FROM z_country WHERE client = @sy-mandt.
    INSERT z_country FROM TABLE @lt_country.

    DATA lt_prod TYPE STANDARD TABLE OF z_product.
    lt_prod = VALUE #(
    ...
    DELETE FROM z_product WHERE client = @sy-mandt.
    INSERT z_product FROM TABLE @lt_prod.

    DATA lt_cust TYPE STANDARD TABLE OF z_customer.
    lt_cust = VALUE #(
    ...
    DELETE FROM z_customer WHERE client = @sy-mandt.
    INSERT z_customer FROM TABLE @lt_cust.
  ENDMETHOD.

  METHOD load_sales.
    DATA: ls_head TYPE z_salesorder,
          ls_item TYPE z_salesitem.

    DELETE FROM z_salesitem WHERE client = @sy-mandt.
    DELETE FROM z_salesorder WHERE client = @sy-mandt.
    ...
    DO 200 TIMES.
      ...
      ls_head-so_id        = |SO{ lv_idx ALPHA = IN }|.
      ls_head-customer_id  = 
        CASE lv_idx MOD 10
      ...
      ls_head-currency_code = 
        CASE lv_idx MOD 9
          ...
          WHEN 6 THEN 'BRL'
          WHEN 7 THEN 'ZAR'
          WHEN OTHERS THEN 'SGD'
        ENDCASE.
      ls_head-total_amount = 0.
      INSERT z_salesorder FROM @ls_head.
      ...
      DO lv_items TIMES.
        ...
        INSERT z_salesitem FROM @ls_item.
        UPDATE z_salesorder SET total_amount = total_amount + @ls_item-net_amount WHERE client = @sy-mandt AND so_id = @ls_head-so_id.
      ENDDO.
    ENDDO.
  ENDMETHOD.
ENDCLASS.
